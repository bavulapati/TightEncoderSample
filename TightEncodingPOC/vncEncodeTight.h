...
public:
	...
	virtual UINT EncodeRect(BYTE *source, char *outfile, BYTE *dest, const RECT &rect, int offx, int offy);

	virtual void SendQueued(char *outfile, const char *buff, const CARD32 bufflen);
...
protected:
	...
	bool CheckSolidTile32 (BYTE *source, int x, int y, int w, int h,
						   CARD32 *colorPtr, bool needSameColor);
	UINT EncodeRectSimple(BYTE *source, char *outfile, BYTE *dest,
		const RECT &rect);
	UINT EncodeSubrect(BYTE *source, char *outfile, BYTE *dest,
