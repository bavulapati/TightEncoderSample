...
void vncEncodeTight::SendQueued(char *outfile, const char *buff, const CARD32 bufflen)
{
	memset(outfile + bufoff, 0x00, bufflen);
	memcpy(outfile + bufoff, buff, bufflen);
	bufoff += bufflen;

	//outfile((char*)&buff, bufofbufflen);	
}


UINT
vncEncodeTight::EncodeRect(BYTE *source, char *outfile, BYTE *dest,
						   const RECT &rect, int offx, int offy)
{
	...
	if (!m_use_lastrect || w * h < MIN_SPLIT_RECT_SIZE)
		return EncodeRectSimple(source, outfile, dest, rect);
	...
	for (dy = y; dy < y + h; dy += MAX_SPLIT_TILE_SIZE) {
		...
		if (dy - y >= nMaxRows) {
			RECT upperRect;
			SetRect((LPRECT)&upperRect, x, y, x + w, y + nMaxRows);

			int size = EncodeRectSimple(source, outfile, dest, upperRect);
			SendQueued(outfile, (char *)dest, size);
		...
		for (dx = x; dx < x + w; dx += MAX_SPLIT_TILE_SIZE) {
			...
			if (CheckSolidTile(source, dx, dy, dw, dh, &colorValue, FALSE)) {
				...
				RECT rects[4];
				SetRect((LPRECT)&rects[0],
						x, y, x + w, y_best);
				SetRect((LPRECT)&rects[1],
						x, y_best, x_best, y_best + h_best);
				SetRect((LPRECT)&rects[2],
						x_best + w_best, y_best, x + w, y_best + h_best);
				SetRect((LPRECT)&rects[3],
				...
				for (int i = 0; i < 4; i++) {
					if (i == 2) {
						...
						SendQueued(outfile, (char *)m_hdrBuffer, m_hdrBufferBytes);
						SendQueued(outfile, (char *)dest, size);
					...
					int size = EncodeRect(source, outfile, dest, rects[i], offsetx, offsety);
					SendQueued(outfile, (char *)dest, size);
				}

				// Return after all recursive calls done (0 == data sent).
				printf("client pixel format is not supported %d\n", bufoff);
...
UINT
vncEncodeTight::EncodeRectSimple(BYTE *source, char *outfile, BYTE *dest,
								 const RECT &rect)
{
	...
	if (w > maxRectWidth || w * h > maxRectSize) {
		...
		for (dy = 0; dy < h; dy += subrectMaxHeight) {
			for (dx = 0; dx < w; dx += maxRectWidth) {
				...
				partialSize = EncodeSubrect(source, outfile, dest,
											x+dx, y+dy, rw, rh);
				if (dy + subrectMaxHeight < h || dx + maxRectWidth < w) {
					SendQueued(outfile, (char *)dest, partialSize);
				}
			}
		}
	} else {
		partialSize = EncodeSubrect(source, outfile, dest, x, y, w, h);
	}
...
UINT
vncEncodeTight::EncodeSubrect(BYTE *source, char *outfile, BYTE *dest,
							 ...
							 int zlibLevel, int zlibStrategy)
{
	...
	if (!m_zsActive[streamId]) {
		...
		printf(("calling deflateInit2 with zlib level:%d\n"),
					 zlibLevel);
		int err = deflateInit2 (pz, zlibLevel, Z_DEFLATED, MAX_WBITS,
								MAX_MEM_LEVEL, zlibStrategy);
		if (err != Z_OK) {
			printf(("deflateInit2 returned error:%d:%s\n"),
						 err, pz->msg);
			return -1;
		}
	...
	if (zlibLevel != m_zsLevel[streamId]) {
		printf(("calling deflateParams with zlib level:%d\n"),
					 zlibLevel);
		int err = deflateParams (pz, zlibLevel, zlibStrategy);
		if (err != Z_OK) {
			printf(("deflateParams returned error:%d:%s\n"),
						 err, pz->msg);
			return -1;
		}
		m_zsLevel[streamId] = zlibLevel;
