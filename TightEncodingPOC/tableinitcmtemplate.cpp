...
rfbInitColourMapSingleTableOUT (char **table,
								...
								rfbPixelFormat *out)
{
	printf(("rfbInitColourMapSingleTable called\n"));
    ...
    *table = (char *)malloc(nEntries * sizeof(OUT_T));
	if (*table == NULL)
	{
		printf(("failed to allocate translation table\n"));
		return;
	}
	...
	if (GetSystemPaletteEntries(hDC,
		0, 256, palette) == 0)
	{
		printf(("failed to get system palette, error=%d\n"),
					 GetLastError());
		ReleaseDC(NULL, hDC);
		return;
	}
	...
	printf(("rfbInitColourMapSingleTable done\n"));
}
