...
void ClientConnection::ReadTightRect(rfbFramebufferUpdateRectHeader *pfburh)
{
  if (m_myFormat.bitsPerPixel != 8 &&
    ...
    printf(("Invalid number of bits per pixel: %d\n"),
    ...
    if ((comp_ctl & 1) && m_tightZlibStreamActive[i]) {
      ...
      if (err != Z_OK) {
        if (m_tightZlibStream[i].msg != NULL) {
          printf(("zlib inflateEnd() error: %s\n"),
                       m_tightZlibStream[i].msg);
        } else {
          printf(("zlib inflateEnd() error: %d\n"), err);
        }
        return;
      }
  ...
  if (compressedLen <= 0) {
    printf(("Tight encoding: bad data received from server.\n"));
    return;
  }
  ...
  if (!m_tightZlibStreamActive[stream_id]) {
    ...
    if (err != Z_OK) {
      if (zs->msg != NULL) {
        printf(("zlib inflateInit() error: %s.\n"), zs->msg);
      } else {
        printf(("zlib inflateInit() error: %d.\n"), err);
      }
      return;
    }
  ...
  if (rowsProcessed != pfburh->r.h) {
    printf(("Tight encoding: wrong number of scan lines.\n"));
  }
}
